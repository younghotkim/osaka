...
export async function GET() {
  ...
  if (error) {
    // osaka_memories not created yet (supabase/init.sql not run) → behave like local mode.
    if (/relation .*osaka_memories.* does not exist|could not find the table/i.test(error.message)) {
      return NextResponse.json({ mode: "local", memories: {} });
    }
