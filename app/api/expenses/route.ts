...
export async function GET() {
  ...
  const { data, error } = await supabase.from("osaka_expenses").select("*").eq("trip_id", tripId);
  if (error) {
    // table not migrated yet — degrade to local instead of 500ing
    if (/relation .*osaka_expenses.* does not exist|could not find the table/i.test(error.message)) {
      return NextResponse.json({ mode: "local", entries: [] });
    }
...
export async function PUT(request: NextRequest) {
  ...
  const supabase = createSupabaseServerClient();
  const del = await supabase.from("osaka_expenses").delete().eq("trip_id", tripId);
  if (del.error) {
    return NextResponse.json({ error: del.error.message }, { status: 500 });
  }
  if (entries.length > 0) {
    const { error } = await supabase.from("osaka_expenses").insert(entries.map(entryToRow) as never);
    if (error) {
      return NextResponse.json({ error: error.message }, { status: 500 });
    }
  }
...
export async function DELETE(request: NextRequest) {
  ...
  const supabase = createSupabaseServerClient();
  const { error } = await supabase.from("osaka_expenses").delete().eq("trip_id", tripId).eq("id", id);
  if (error) {
    return NextResponse.json({ error: error.message }, { status: 500 });
  }
  return NextResponse.json({ mode: "supabase", saved: true });
