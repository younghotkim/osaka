...
export async function GET() {
  ...
  const { data, error } = await supabase
    ...
    .eq("trip_id", tripId);
  if (error) {
    if (/relation .*osaka_vault.* does not exist|could not find the table/i.test(error.message)) {
      return NextResponse.json({ mode: "local", items: [] });
    }
...
export async function PUT(request: NextRequest) {
  ...
  const supabase = createSupabaseServerClient();
  const del = await supabase.from("osaka_vault").delete().eq("trip_id", tripId);
  if (del.error) return NextResponse.json({ error: del.error.message }, { status: 500 });
  if (items.length > 0) {
    const { error } = await supabase.from("osaka_vault").insert(items.map(itemToRow) as never);
