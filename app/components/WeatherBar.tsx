...
export function useWeatherForecast(coords: { lat: number; lng: number } = OSAKA) {
...
export function WeatherBar({ coords }: { coords?: { lat: number; lng: number } }) {
  const { forecast, loading } = useWeatherForecast(coords ?? OSAKA);
...
export function useDailyForecast(coords: { lat: number; lng: number } = OSAKA) {
