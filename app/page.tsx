...
import { OsakaInfo } from "./components/OsakaInfo";
...
const storageKey = "yj-osaka-memory-book-v1";
const modeStorageKey = "yj-osaka-mode-v1";
...
function HomeShell() {
  ...
  const addStop = useCallback(async () => {
    ...
    const newStop: TripStop = {
      ...
      phrase: "",
      category: "sight",
      lat: anchor?.lat ?? 34.6687,
      lng: anchor?.lng ?? 135.5013,
      highlights: [],
      prompt: "",
      mapsQuery: ""
    };
  ...
  // Drag a marker on the map → persist the stop's new coordinates immediately.
  const moveStopPin = useCallback(
    async (stopId: string, lat: number, lng: number) => {
      const stop = tripStops.find((s) => s.id === stopId);
      if (!stop) return;
      const dayList = tripStops.filter((s) => s.day === stop.day);
      const index = dayList.findIndex((s) => s.id === stopId);
      await itinerary.saveStop({ ...stop, lat, lng }, (index < 0 ? dayList.length : index) * 100);
    },
    [tripStops, itinerary]
  );

  // Tap an empty spot on the map (pending pin) → create a new stop right there.
  const addStopAt = useCallback(
    async (lat: number, lng: number) => {
      const day = activeDay || 1;
      const dayStops = dayStopsInOrder(day);
      const id = newStopId();
      const newStop: TripStop = {
        id,
        day,
        date: tripDays.find((d) => d.day === day)?.date ?? "",
        time: "",
        title: "새 스톱",
        subtitle: "",
        nameZh: "",
  ...
  const exportMemories = () => {
    ...
    link.href = url;
    link.download = "yj-osaka-memories.json";
    link.click();
  ...
  return (
    <main className={`app-shell app-shell--${mode}`}>
      <header className="topbar">
        <div className="brand">
          <span className="brand__mark" aria-label="일본 국기 - 오사카 둘만의 여행">
            <JapanFlag className="brand__flag" />
            <span className="brand__mark-label">OSAKA</span>
          </span>
          <div className="brand__copy">
            <strong>
              <span className="brand__zh">Y&amp;S</span>
              <span className="brand__en">大阪</span>
            </strong>
            <small>
      ...
      {mode === "plan" && (
        <PlanShell
          ...
          onMoveStopPin={moveStopPin}
          onAddStopAt={addStopAt}
          onMoveStop={moveStop}
      ...
      <section className={`workspace workspace--view-${planView}`}>
        ...
        <section className="map-stage" aria-label="지도">
          <MapView
            ...
            editMode={editMode}
            dayFilter={activeDay}
            onMoveStopPin={onMoveStopPin}
          />
          <div className="map-floating map-floating--top">
            <div>
              <span>{activeDay === 0 ? "전체 동선" : `Route ${activeDay}`}{editMode ? " · 편집 중" : ""}</span>
              <strong>{activeDay === 0 ? `${tripDays.length}일` : activeDayData.date}</strong>
            </div>
            <a
          ...
          {editMode && pendingPin && (
            <button
              className="map-floating map-floating--add"
              onClick={() => onAddStopAt(pendingPin.lat, pendingPin.lng)}
            >
              <Plus size={16} />
              이 핀 위치에 새 스톱 추가 (Day {activeDay || 1})
            </button>
          )}
          <div className="map-floating map-floating--bottom">
            ...
            <span>
              {editMode
                ? pendingPin
                  ? "핀 위치로 새 스톱을 만들거나, 편집 패널에서 좌표 적용"
                  : "핀을 길게 끌면 위치 이동 · 빈 곳을 탭하면 새 핀"
        ...
        {importOpen && (
          <div className="import-box">
            <textarea
              ...
              placeholder="yj-osaka-memories.json 내용을 붙여넣기"
            />
    ...
    <section className="vault-stage">
      <header className="vault-head">
        <div>
          ...
          <h1>준비물 · 예약 · 정보</h1>
          <p>항공·숙소·예약 보관함, 챙길 준비물 체크리스트, 오사카 현지 정보를 한 곳에.</p>
        </div>
